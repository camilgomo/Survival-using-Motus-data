...
surv<-read.csv("C:\\Users\\cg478\\Documents\\Camila\\Publicaciones\\Survival Migration MOTUS\\scripts\\SurvivalMOTUS\\allSpp.motus.jags.csv",header=TRUE)
...
nind<-nrow(y)
group <- surv[, "Species"]
group <- ifelse(surv[,"Species"]=="SWTH",1,ifelse(surv[,"Species"]=="BLPW",2,ifelse(surv[,"Species"]=="GCTH",3,4)))
...
##### Interaction species x time DOES NOT CONVERGE
...
for (i in 1:nind){
   for (t in f[i]:(n.occasions-1)){
      phi[i,t] <- beta.phi[group[i],t]
      p[i,t] <- beta.p[group[i],t]
      } #t
   } #i
# for survival parameters
for (u in 1:g){
   for (t in 1:(n.occasions-1)){
      beta.phi[u,t] ~ dunif(0, 1)     # Prior for time and group-spec. survival
      } #t
   } #g
# for recapture parameters
for (u in 1:g){
   for (t in 1:(n.occasions-1)){
      beta.p[u,t] ~ dunif(0, 1)     # Prior for time and group-spec. recapture
      } #t
   } #g
...
windows()
plot(x=(1:(n.occasions-1))-0.1, y = cjs.add$BUGSoutput$mean$phi.g1, type = "b", pch = 16, ylim = c(0, 1), ylab = "Survival probability", xlab = "Latitude", bty = "n", cex = 1.5, axes = FALSE)
axis(1, at = 1:6, labels = rep(NA,6), tcl = -0.25)
axis(1, at = seq(1,6,1), labels = c("4","11","29","39","49", ">59"))
axis(2, at = seq(0.2, 1, 0.1), labels = c("0.2", NA, "0.4", NA, "0.6", NA, "0.8", NA, "1.0"), las = 1)
segments((1:(n.occasions-1))-0.1, lower.f, (1:(n.occasions-1))-0.1, upper.f)
