...
blpwsurv<-read.csv("C:\\Users\\camil\\OneDrive\\Documents\\Camila\\Publicaciones\\Survival Migration MOTUS\\scripts\\SurvivalMOTUS\\blwa.motus.jags.csv",header=TRUE)
blpwsurv<-as.matrix(blpwsurv[,6:11])
y<-blpwsurv
time<-seq(1:6)
ntime<-length(time)
nind<-nrow(y)
...
# Call JAGS from R 
BLWA<- jags(jags.data, inits, parameters, "BLWA.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, DIC=TRUE)
...
# Call JAGS from R 
BLWA2<- jags(jags.data, inits, parameters, "BLWA2.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, DIC=TRUE)

# Summarize posteriors
print(BLWA2, digits = 3)
