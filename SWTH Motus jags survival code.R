...
swthsurv<-read.csv("C:\\Users\\camil\\OneDrive\\Documents\\Camila\\Publicaciones\\Survival Migration MOTUS\\scripts\\SurvivalMOTUS\\swth.motus.jags.ANASURV.csv",header=TRUE, sep = ";")
swthsurv<-as.matrix(swthsurv[,5:10])
y<-swthsurv
time<-seq(1:6)
ntime<-length(time)
nind<-nrow(y)
...
for (i in 1:nind){
	...
	first[i] <- min((1:6)[!is.na(h) & h==1]) # for each individual find occasion of first marking
      last[i]  <- max((1:6)[!is.na(h) & h==1]) # for each individual find occasion of last observation
...
p0[6]~dunif(0.2,0.4) # contrained uniform prior for last interval to provide better estimate of survival 

mig.surv1<-phi0[2]*phi0[3]*phi0[4]*phi0[5]*phi0[6] # calculate survival across all stages
mig.surv2<-phi0[2]*phi0[3]*phi0[4]*phi0[5] # survival across all but final stage 
...
# Call JAGS from R 
SWTH<- jags(jags.data, inits, parameters, "SWTH.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, DIC=TRUE)

# Summarize posteriors
print(SWTH, digits = 3)

######################Model with priors for detectability set according to values from survivors.
...
# Call JAGS from R 
SWTH2<- jags(jags.data, inits, parameters, "SWTH2.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, DIC=TRUE)

# Summarize posteriors
print(SWTH2, digits = 3)
